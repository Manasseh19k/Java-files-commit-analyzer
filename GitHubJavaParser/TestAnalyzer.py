...
class TestAnalyzer:
    def __init__(self, parent_commit_sha, project_name):
        ...
        self.repo = None
        self.reserved_words = ['assertEquals', 'fail', 'assertTrue', 'assertFalse', 'assertNull', 'assertThat', 'assertThrows', 'assertNumberOfNodes', 'assertNotNull', 'assertSame']
    ...
    def analyze_method_calls(self, tree, method_name, class_methods_map, variable_class_map, internal_methods_map, class_import_map):
        method_node = next(
            (node for _, node in tree.filter(javalang.tree.MethodDeclaration) if node.name == method_name), None)
        if method_node:
            self.process_method_body(method_node, class_methods_map, variable_class_map, internal_methods_map, class_import_map)

    def process_method_body(self, method_node, class_methods_map, variable_class_map, internal_methods_map, class_import_map):
        for _, invocation_node in method_node.filter(javalang.tree.MethodInvocation):
            self.process_method_invocation(invocation_node, class_methods_map, variable_class_map, internal_methods_map, class_import_map)
            ...
            if test_file_path:
                ...
                for class_name, method_calls in class_method_mappings.items():
                    results.append({
                        ...
                        'Method Calls': ', '.join(method_calls),
    ...
    def write_to_csv(self, results, output_csv):
        with open(output_csv, mode='w', newline='', encoding='utf-8') as csv_file:
            fieldnames = ['Test Case Link', 'Class Name', 'Method Calls', 'Redundant Test Name', 'Commit SHA']
...
# Example usage
parent_commit_sha = "bf549f0589105a42f159c78a621fc28087561d16"
project_name = "google/gson"
redundant_test_names = ['testFieldAdapterInvoked', 'testFieldAdapterNotInvokedIfNull', 'testNonStaticFieldAdapterNotInvoked', 'testIncorrectTypeAdapterNotInvoked', 'testSuperclassTypeAdapterNotInvoked']
output_csv = "redundant_tests_with_classes.csv"
